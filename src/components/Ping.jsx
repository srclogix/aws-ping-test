...
const Ping = () => {
  ...
  const checkLatency = (index) => {
    ...
    else addPingResult(index, null);
  };

  const cloudPingTest = async () => {
    const pingRequests = pingResults.map((region, index) =>
      checkLatency(index)
    );
  ...
  useEffect(() => {
    ...
    if (pingResults?.length === TOTAL_REGIONS) setIsReady(true);
    if (pingResults?.every((region) => region.pings.length === TOTAL_PINGS))
      setIsFinished(true);
  }, [pingResults]);

  useEffect(() => {
    if (isReady) startPinging(TOTAL_PINGS);
  }, [isReady]);

  const calculateRecommendRegion = () => {
    const latencies = pingResults
      .map((region) => region.latency)
      .filter((latency) => latency);
    const minLatency = Math.min(...latencies);
    const minLatencyIndex = pingResults.findIndex(
      (region) => region.latency == minLatency
    );
  ...
  useEffect(() => {
    if (isFinished) calculateRecommendRegion();
  }, [isFinished]);
        ...
        <div className="d-flex">
          <h4 className="header-title">
            ...
            <p>
              ...
              across the globe. Help user pick the most optimal media region.
            </p>
          </h4>
          {/* refresh button */}
          {isFinished && (
            ...
            <div className="col-md-4" key={nanoid()}>
              <div
                className={`green-border ${
                  recommendedRegion === index ? "" : "gray-border"
                }`}
              >
                ...
                {isReady && !isFinished && (
                  <div className="progress">
                    <span
                      ...
                      style={{ width: `${region.pings.length * TOTAL_PINGS}%` }}
                    />
                  </div>
                )}
