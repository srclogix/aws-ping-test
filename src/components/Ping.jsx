...
const getLatencyStyle = (time) => {
  if (!time || time > 4000) return "red";
  else if (time >= 2000 && time <= 4000) return "orange";
};
const checkAvgLatency = (list) => {
  return Math.round(
    list.reduce((sum, num) => sum + (num ? num : 0), 0) / list.length
  );
};
const PingComponent = () => {
  ...
  const addPingResult = (index, time) => {
    ...
    newRegionMap.latency = time
      ? checkAvgLatency(newRegionMap.pings)
      : newRegionMap.latency;
  ...
  useEffect(() => {
    if (isReady && pingCount != TOTAL_PINGS) startPinging();
  }, [isReady, pingCount]);
